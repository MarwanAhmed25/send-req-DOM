//import get_random_cook from "./cooki";
...
class Table {
  ...
  clicked_button(e) {
    ...
    var td = document.createElement("TD");
    td.innerText = 0;
    td.setAttribute('id',`num_of_req${count}`);
    count +=1;
    this.row.appendChild(td);
    
    document.getElementById("results").appendChild(this.row);
        this.run();
    this.clear__();
    return true;
  }
...
document.getElementById("submit").addEventListener("click", (e) => {
  let t = new Table();
  arr_of_ids_result.push(t);
  var b = t.clicked_button(e);
  if(b){
    for(let i=0; i< arr_of_ids_result.length; i++){
          arr_of_ids_result[i].run();
    }
  }
});
...
function fetch_data(){
    ...
    d = document.getElementById(`num_of_req${count-1}`);
    d.innerText = 1 + Number(d.innerText);
...
function random_num() {
  return Math.floor(Math.random() * 100) % 29;
}

function get_random_cook() {
  var n = random_num();
  return cook[n].cook_info;
}
